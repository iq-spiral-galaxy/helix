...
- 상세 화면에서 현재 나선의 열린 질문 1개와 나머지를 정확히 펼쳐 보고, 긴 질문도 잘림 없이 읽을 수 있습니다.
- Layer와 열린 질문을 숫자가 결합된 선형 마크로 통일하고, 단일 Layer에서는 불필요한 이동 제어를 숨깁니다.
- 질문 검색·정렬과 지도 목록 제어를 더 크고 명료하게 다듬고, 나선 지도 재생 속도를 두 배로 높였습니다.
- 표현 깊이를 색과 아이콘이 함께 있는 배지로 구분하고 전체 대화의 불필요한 메시지 수를 제거했습니다.
