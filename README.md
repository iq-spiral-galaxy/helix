...
뷰어는 옵시디언풍 앱 셸(사이드바 + 탭 워크스페이스)의 정적 SPA다. 차분한 다크·라이트 테마와 무채색 위계에 액센트 2색(파랑 = 현재 위치, 세이지 = 관계)만 쓴다.
...
- **일관된 계층 탐색** — 컬렉션(Repo) → Chapter → Subject 구조를 사이드바·지도·상세 breadcrumb에서 동일하게 유지
- **연결 (옵시디언식)** — 명시 edge가 없어도 로드맵 형제 + 태그 공유(IDF 랭킹)에서 연결을 파생. 상세 페이지 연결 패널, 지도의 호버 점등으로 표면화
- **나선 지도** — 아르키메데스 나선 위 등호장 배치, 스프링 복원·드래그, 휠 줌·팬, 화면 맞춤, 정착 시 rAF 주차(idle CPU 0)
...
- **모바일 탐색** — 검색·탭·나선 목록을 유지하는 드로어와 하단 내비게이션, 목록 우선 지도 탐색
